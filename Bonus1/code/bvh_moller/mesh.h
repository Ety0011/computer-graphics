...
struct BVHNode
{
  ...
  std::vector<Triangle> triangles; // TODO change this
...
public:
  ...
  BVHNode *buildBVH(std::vector<Triangle> &triangles)
  {
    ...
    std::sort(
        ...
        {
          return a.boundingBox.minBounds.x < b.boundingBox.minBounds.x;
        });
  ...
  Hit traverseBVH(BVHNode *node, const Ray &ray)
  {
    ...
    raym::vec3 tOrigin = raym::to_vec3_perspective_safe(inverseTransformationMatrix * raym::vec4(ray.origin, 1.0));
    raym::vec3 tDirection = raym::to_vec3_perspective_safe(inverseTransformationMatrix * raym::vec4(ray.direction, 0.0));
    tDirection = raym::normalize(tDirection);
...
inline std::vector<Object *> defineObjects(const std::vector<std::string> &meshFilenames)
{
  ...
  for (size_t i = 0; i < nMeshes; i++)
  {
    ...
    raym::mat4 translation = raym::translate(raym::vec3(x_pos, -1.0f, 5.0f));
